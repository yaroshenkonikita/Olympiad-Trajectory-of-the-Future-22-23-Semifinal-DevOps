...
TEST(Chess, under_attack_on_diagonally_x_plus_y_plus) {
    ...
    EXPECT_EQ(clear_field.IsUnderAttack(positionKingX, positionKingY), Piece::PieceFigure::BISHOP);
}
...
TEST(Chess, under_attack_on_horizontally_x_plus) {
    ...
    EXPECT_EQ(clear_field.IsUnderAttack(positionKingX, positionKingY), Piece::PieceFigure::ROOK);
}
